...
function check_names(component, env) {
  is_literal(component)
    ...
    ? check_names(
        make_sequence(
          pair(function_expression(component), arg_expressions(component))
        ),
        env
      )
    ...
    ? check_names(
        ...
        extend_environment(
          ...
          list_of_unassigned(lambda_parameter_symbols(component)),
          env
        )
      )
    : is_sequence(component)
    ? map((stmt) => check_names(stmt, env), sequence_statements(component))
    : is_block(component)
    ? check_names(
        block_body(component),
        extend_environment(
          scan_out_declarations(block_body(component)),
          list_of_unassigned(scan_out_declarations(block_body(component))),
          env
        )
      )
    ...
    ? check_names(
        make_sequence(
          list(
            make_name(assignment_symbol(component)),
    ...
    ? check_names(
        make_sequence(
          list(
            make_name(declaration_symbol(component)),
...
function operator_combination_to_application(component) {
  ...
  return operator === "!" || operator === "-unary"
    ...
    : make_application(
        make_name(operator),
        list(
          operator_combination_first_operand(component),
...
function parse_and_evaluate(input) {
  ...
  return evaluate(implicit_top_level_block, the_global_environment);
}
...
// function sort_function_declarations(stms) {

//     function get_functions(stms) {
//         return filter(stm => is_function_declaration(stm), stms);
//     }

//     function remove_functions(stms) {
//         return filter(stm => !is_function_declaration(stm), stms);
//     }

//     return append(get_functions(stms), remove_functions(stms));

// }

// function eval_sequence(stmts, env) {
//     stmts = sort_function_declarations(stmts);
//     if (is_empty_sequence(stmts)) {
//         return undefined;
//     } else if (is_last_statement(stmts)) {
//         return evaluate(first_statement(stmts),env);
...
// CHANGED HERE:
// unassigned refers to a Source value
// that is guaranteed to be unaccessible
// by interpreted programs. Any reference
// would do the job: a pair, array or
// function object
...
function lookup_symbol_value(symbol, env) {
    ...
    function scan(symbols, vals) {
      return is_null(symbols)
        ...
        ? // CHANGED HERE:
          ...
          ? error(symbol, "Cannot access name before initialization:")
          : head(vals)
        : scan(tail(symbols), tail(vals));
    }
    if (env === the_empty_environment) {
  ...
  const value = env_loop(env);
  if (value === "*unassigned*") {
    return error("Name accessed before initialisation");
  } else {
    return value;
  }
}
